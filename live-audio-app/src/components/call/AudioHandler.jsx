...
const AudioHandler = ({ role }) => {
  // Determine if this instance should initiate the connection
  const isInitiator = role === 'agent';
  
  const { setupMedia, remoteStreamRef, cleanup } = useWebRTC(isInitiator);
  ...
  const recordingStateRef = useRef(false); // Prevent state race conditions
  ...
  useEffect(() => {
    console.log('🎯 AudioHandler starting for role:', role, 'isInitiator:', isInitiator);
    ...
    const initializeConnection = async () => {
      try {
        ...
        // Start monitoring for remote stream
  ...
  }, [role, setupMedia, cleanup, updateStatus]);
  ...
  const startRemoteStreamMonitoring = useCallback(() => {
    ...
    checkIntervalRef.current = setInterval(() => {
      ...
      if (remoteStream && remoteStream.getAudioTracks().length > 0) {
        ...
        if (attempts % 10 === 0) { // Detailed logging every 10 attempts
        ...
        if (activeTracks.length > 0) {
          ...
          // Auto-start recording for agent role
          if (role === 'agent') {
  ...
  const startRecording = useCallback((streamToRecord = null) => {
    ...
    try {
      ...
      // Auto-stop recording after 60 seconds (adjust as needed)
